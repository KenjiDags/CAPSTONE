...
// If an item_id is provided, return that item's monthly history (stock card)
if (!empty($_GET['item_id'])) {
    $item_id = intval($_GET['item_id']);
    $stmt = $conn->prepare("SELECT DATE_FORMAT(changed_at, '%Y-%m') AS month, MAX(quantity_on_hand) AS qty
        FROM item_history
        WHERE item_id = ? AND changed_at >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
        GROUP BY month
        ORDER BY month");
